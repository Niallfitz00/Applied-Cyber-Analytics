...
wordcloud_data <- read.csv("C:/Cybersecurity Masters/Blair Applied Analytics/Assignment 3/Assignment3Project/wordcloudfile.csv", stringsAsFactors = FALSE)
converted_text <- iconv(wordcloud_data$text, to = "UTF-8")
# Assuming wordcloud_data is your original data frame and 'text' is the column you want to convert
...
leaflet(world_data) %>%
  ...
  addLegend(position = "bottomright",
            ...
            opacity = 1)
#-------------------------------------------------------------------------------
#Predicting the Severity of Data Breaches Based
# on Country-Specific Factors and Breach Characteristics
#Using Merged Countries and DataBreaches Datasets

# Convert to consistent case for joining
countries <- countries %>% mutate(CountryName = tolower(CountryName))
databreaches <- databreaches %>% mutate(CountryName = tolower(CountryName))
# Join the datasets
combined_data <- merge(countries, databreaches, by = "CountryName")
# Feature engineering example:
# You may want to turn 'Records' into a numerical feature that indicates severity
combined_data$Severity <- ifelse(combined_data$Records > 1000000, 'High', ifelse(combined_data$Records > 50000, 'Medium', 'Low'))

combined_data$CountryName <- as.factor(combined_data$CountryName)
combined_data$Sector <- as.factor(combined_data$Sector)
combined_data$Method <- as.factor(combined_data$Method)
combined_data$Severity <- as.factor(combined_data$Severity)

# Split data into training and testing sets
set.seed(123) # Ensure reproducibility
index <- createDataPartition(combined_data$Severity, p = 0.8, list = FALSE)
trainData <- combined_data[index, ]
testData <- combined_data[-index, ]

# Exclude 'Entity' from both trainData and testData
trainData <- select(trainData, -Entity)
testData <- select(testData, -Entity)

# Model Training
# Specify the predictive model formula excluding 'Entity'
model_formula <- Severity ~ CEI + GCI + NCSI + DDL + ThreatLevel + Classification + Year + Records + Sector + Method

# Train the model
model <- train(model_formula, data = trainData, method = 'rpart', trControl = trainControl(method = "cv"))

# Make predictions on the test data
predictions <- predict(model, newdata = testData)

# Evaluate the model's predictions using confusionMatrix
conf_matrix <- confusionMatrix(predictions, testData$Severity)

# Print the confusion matrix in a detailed, presentable way
print(conf_matrix)
...
docs <- tm_map(docs, content_transformer(function(x) iconv(x, to = "UTF-8"))) # Ensure UTF-8
...
df_word_freqs <- df_word_freqs[!duplicated(df_word_freqs$word), ]

# Define your specific color palette
my_colors <- c("green", "red", "blue", "pink", "purple", "black")

# Ensure each word gets a color from the palette
# The colors will repeat if there are more words than colors
color_assignment <- my_colors[(seq_len(nrow(df_word_freqs)) - 1) %% length(my_colors) + 1]

# Adjusting the dimensions for a bigger image
png("wordcloud_custom_colors.png", width = 1280, height = 960)

# Generate the word cloud with larger font sizes
wordcloud(words = df_word_freqs$word, freq = df_word_freqs$freq,
          min.freq = 1, max.words = nrow(df_word_freqs), random.order = FALSE,
          rot.per = 0.35, colors = color_assignment,
          scale = c(11, 3)) # Larger scale for bigger font sizes
